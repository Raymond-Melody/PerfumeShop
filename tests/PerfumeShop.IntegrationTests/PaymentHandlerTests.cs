...
/// <summary>
/// PaymentHandler 单元测试 — 基于 V18 payment_handler.asp 真实业务数据回归
...
public class PaymentHandlerTests : IDisposable
{
    ...
    public PaymentHandlerTests()
    {
        var options = new DbContextOptionsBuilder<PerfumeShopContext>()
            .UseInMemoryDatabase(databaseName: $"PayHandlerTests_{Guid.NewGuid()}")
            .Options;
    ...
    private async Task SeedOrderWithDetailsAsync(int orderId, int productId, int qty)
    {
        _db.OrderDetails.Add(new OrderDetail
        {
            ...
            UnitPrice = 100m, Subtotal = 100m * qty,
    ...
    /// <summary>测试3: 无效支付方式 — 返回失败</summary>
    [Fact]
    ...
    /// <summary>测试4: 正常支付回调 — 状态更新为 Paid</summary>
    [Fact]
    ...
    /// <summary>测试5: 重复回调（幂等性） — 第二次调用返回 IsDuplicate=true</summary>
    [Fact]
    ...
    /// <summary>测试6: 无效 order_id — 返回失败</summary>
    [Fact]
    ...
    /// <summary>测试8: 高退货率用户 — 风控不通过</summary>
    [Fact]
    public async Task CheckRisk_HighReturnRate_Fails()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            _db.Orders.Add(new Order
            {
                OrderNo = $"R{i}", UserId = 2, TotalAmount = 50,
    ...
    /// <summary>测试10: 幂等性 — 重复调用不创建重复工单</summary>
    [Fact]
    public async Task AutoCreateProductionOrder_Idempotent_NoDuplicates()
    {
        ...
        Assert.Equal(3, count1); // 3瓶
    }

    /// <summary>测试11: 无订单明细时 — 返回 true 不创建工单</summary>
    [Fact]
    ...
    /// <summary>测试12: ConfirmPayment — 整合幂等+状态+生产工单</summary>
    [Fact]
    ...
    /// <summary>测试13: 状态同步 — Pending→Paid 触发生产工单</summary>
    [Fact]
    ...
    /// <summary>测试14: 接口方法签名覆盖 V18 所有支付函数</summary>
    [Fact]
