...
/// <summary>
/// PromotionEngine 单元测试 — 基于 V18 promotion_engine.asp 真实业务数据回归
/// 使用 EF Core InMemory Provider
/// </summary>
public class PromotionEngineTests : IDisposable
{
    ...
    public PromotionEngineTests()
    {
        var options = new DbContextOptionsBuilder<PerfumeShopContext>()
            .UseInMemoryDatabase(databaseName: $"PromoEngineTests_{Guid.NewGuid()}")
            .Options;
    ...
    private async Task SeedCouponAsync(string code, string type, decimal value, decimal minSpend = 0,
        ...
        int totalQty = 100, int usedQty = 0, bool isActive = true)
    {
        _db.Coupons.Add(new Coupon
        {
            ...
            TotalQty = totalQty, UsedQty = usedQty, IsActive = isActive,
    ...
    private async Task SeedUserAsync(int userId, string? tier = null)
    {
        ...
        {
            UserId = userId, Username = $"user{userId}", Email = $"user{userId}@test.com",
            Password = "hashed", CustomerTier = tier, CreatedAt = DateTime.Now
        });
        await _db.SaveChangesAsync();
    }
    ...
    /// <summary>测试1: Fixed 满减券 — 减固定金额</summary>
    [Fact]
    public async Task ValidateCoupon_Fixed_ReturnsFixedDiscount()
    {
        await SeedCouponAsync("FIXED50", "fixed", 50, minSpend: 200);
        await SeedUserAsync(1);

        var result = await _engine.ValidateCouponAsync("FIXED50", 1, 300);
    ...
    /// <summary>测试2: Percentage 折扣券 — 按百分比折扣，受 maxDiscount 限制</summary>
    [Fact]
    public async Task ValidateCoupon_Percentage_CappedAtMaxDiscount()
    {
        ...
        Assert.True(result.Valid);
        Assert.Equal(50m, result.Discount); // 被 maxDiscount 限制
        Assert.Equal("percentage", result.Type);
    }

    /// <summary>测试3: FreeShipping 免邮券 — 折扣金额为0</summary>
    [Fact]
    ...
    /// <summary>测试4: Gift 礼品券 — 折扣金额为0</summary>
    [Fact]
    ...
    /// <summary>测试5: 过期优惠券 — 应返回无效</summary>
    [Fact]
    ...
    /// <summary>测试6: 首单限制 — 已有订单的用户不能使用</summary>
    [Fact]
    ...
    /// <summary>测试7: 库存耗尽 — 已领完的优惠券不可使用</summary>
    [Fact]
    ...
    /// <summary>测试8: 品类限制 — 不匹配的品类应被拒绝</summary>
    [Fact]
    ...
    /// <summary>测试9: 品类匹配 — 应通过验证</summary>
    [Fact]
    public async Task ValidateCoupon_CategoryMatch_ReturnsValid()
    {
        await SeedCouponAsync("CATOK", "percentage", 15, category: "女士香水");
        await SeedUserAsync(1);

        var result = await _engine.ValidateCouponAsync("CATOK", 1, 300, cartCategory: "女士香水");

        Assert.True(result.Valid);
    }

    /// <summary>测试10: 最低消费未达 — 应返回无效</summary>
    [Fact]
    ...
    /// <summary>测试11: 已使用过的优惠券 — 不可重复使用</summary>
    [Fact]
    ...
    /// <summary>测试12: 满减促销 — 达到门槛应享受折扣</summary>
    [Fact]
    ...
    /// <summary>测试13: 免运费检查 — 达到门槛返回 true</summary>
    [Fact]
    ...
    /// <summary>测试14: ApplyDiscount — 促销+优惠券叠加，不超过50%上限</summary>
    [Fact]
    public async Task ApplyDiscount_CombinedRespectsMaxRatio()
    {
        ...
        Assert.True(result.TotalDiscount <= 50m); // 不超过50%
        Assert.True(result.FinalAmount >= 50m);
    }

    // ==================== 5. 优惠券发放与查询 ====================

    /// <summary>测试15: IssueCoupon + GetUserCoupons — 发放后可查询</summary>
    [Fact]
    ...
    /// <summary>测试16: GetPromotionHistoryByUser — 返回用户历史</summary>
    [Fact]
