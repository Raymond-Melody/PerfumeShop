     ...
     ============================================ -->
        <div class="checkout-content">
            <div class="checkout-items">
                <h3><% If FEATURE_I18N Then %><%= T("checkout_items_title", Empty) %><% Else %>订单商品<% End If %></h3>
                ...
                <div class="checkout-item">
                    ...
                    <div class="item-details">
                        ...
                        <div class="item-attributes">
                            ...
                            <span><i class="fas fa-wind"></i> <% If FEATURE_I18N Then %><%= T("product_notes_top_title", Empty) %><% Else %>前调<% End If %>: <%= chkTopList %></span>
                            <% ElseIf chkProductTypeLC = "custom" And Not IsNull(rsCart("TopNoteName")) Then %>
                            <span><i class="fas fa-wind"></i> <% If FEATURE_I18N Then %><%= T("product_notes_top_title", Empty) %><% Else %>前调<% End If %>: <%= HTMLEncode(rsCart("TopNoteName")) %></span>
                            <% End If %>
                            <% If chkProductTypeLC = "custom" And chkMidList <> "" Then %>
                            <span><i class="fas fa-heart"></i> <% If FEATURE_I18N Then %><%= T("product_notes_mid_title", Empty) %><% Else %>中调<% End If %>: <%= chkMidList %></span>
                            <% ElseIf chkProductTypeLC = "custom" And Not IsNull(rsCart("MiddleNoteName")) Then %>
                            <span><i class="fas fa-heart"></i> <% If FEATURE_I18N Then %><%= T("product_notes_mid_title", Empty) %><% Else %>中调<% End If %>: <%= HTMLEncode(rsCart("MiddleNoteName")) %></span>
                            <% End If %>
                            <% If chkProductTypeLC = "custom" And chkBaseList <> "" Then %>
                            <span><i class="fas fa-moon"></i> <% If FEATURE_I18N Then %><%= T("product_notes_base_title", Empty) %><% Else %>后调<% End If %>: <%= chkBaseList %></span>
                            <% ElseIf chkProductTypeLC = "custom" And Not IsNull(rsCart("BaseNoteName")) Then %>
                            <span><i class="fas fa-moon"></i> <% If FEATURE_I18N Then %><%= T("product_notes_base_title", Empty) %><% Else %>后调<% End If %>: <%= HTMLEncode(rsCart("BaseNoteName")) %></span>
                            <% End If %>
                            <% If Not IsNull(rsCart("VolumeName")) Then %>
                            <span><i class="fas fa-tint"></i> <% If FEATURE_I18N Then %><%= T("product_option_volume", Empty) %><% Else %>容量<% End If %>: <%= rsCart("VolumeML") %>ml (<%= HTMLEncode(rsCart("VolumeName")) %>)</span>
                            <% End If %>
                            <% If Not IsNull(rsCart("BottleName")) Then %>
                            <span><i class="fas fa-wine-bottle"></i> <% If FEATURE_I18N Then %><%= T("product_option_bottle", Empty) %><% Else %>瓶身<% End If %>: <%= HTMLEncode(rsCart("BottleName")) %></span>
                            <% End If %>
                            <% If Not IsNull(rsCart("CustomLabel")) And rsCart("CustomLabel") <> "" Then %>
                            <span><i class="fas fa-pen-fancy"></i> <% If FEATURE_I18N Then %><%= T("order_ingredients_engraving", Empty) %><% Else %>刻字<% End If %>: <%= HTMLEncode(rsCart("CustomLabel")) %></span>
                            <% End If %>
                            <% 
                            ' 显示刻字费用
                            ...
                            %>
                            <span style="color:#e91e63;"><i class="fas fa-tag"></i> <% If FEATURE_I18N Then %><%= T("cart_summary_engraving", Empty) %><% Else %>刻字费用<% End If %>: <%= FormatMoney(checkoutItemEngravingPrice) %></span>
                            <% End If %>
                        </div>
                    </div>
