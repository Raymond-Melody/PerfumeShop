...
<%
Response.Redirect "purchase_orders.asp?new=1&order_type=SprayHead"
%>
...
' 确保 SprayHeadInventory 表存在
On Error Resume Next
conn.Execute "SELECT TOP 1 1 FROM SprayHeadInventory"
If Err.Number <> 0 Then
    Err.Clear
    conn.Execute "CREATE TABLE SprayHeadInventory (SprayHeadID INT IDENTITY(1,1) PRIMARY KEY, ItemName NVARCHAR(200) NOT NULL, ItemCode NVARCHAR(50), SprayType NVARCHAR(30) DEFAULT 'Mist', Material NVARCHAR(100), Specification NVARCHAR(200), Unit NVARCHAR(20) DEFAULT 'pcs', StockQty FLOAT DEFAULT 0, SafetyStock FLOAT DEFAULT 0, UnitPrice DECIMAL(19,4) DEFAULT 0, WeightedUnitCost DECIMAL(19,4) DEFAULT 0, SupplierID INT, LastPurchaseDate DATETIME, Notes NVARCHAR(500), IsActive BIT DEFAULT 1, CreatedAt DATETIME DEFAULT GETDATE(), UpdatedAt DATETIME)"
End If

' 确保字段存在（兼容旧版不同结构）
conn.Execute "SELECT SupplierID FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD SupplierID INT"
conn.Execute "SELECT IsActive FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD IsActive BIT DEFAULT 1"
conn.Execute "SELECT ItemCode FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD ItemCode NVARCHAR(50)"
conn.Execute "SELECT SprayType FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD SprayType NVARCHAR(30) DEFAULT 'Mist'"
conn.Execute "SELECT Material FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD Material NVARCHAR(100)"
conn.Execute "SELECT Specification FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD Specification NVARCHAR(200)"
conn.Execute "SELECT Unit FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD Unit NVARCHAR(20) DEFAULT 'pcs'"
conn.Execute "SELECT UnitPrice FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD UnitPrice DECIMAL(19,4) DEFAULT 0"
conn.Execute "SELECT WeightedUnitCost FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD WeightedUnitCost DECIMAL(19,4) DEFAULT 0"
conn.Execute "SELECT Notes FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD Notes NVARCHAR(500)"
conn.Execute "SELECT LastPurchaseDate FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD LastPurchaseDate DATETIME"
conn.Execute "SELECT CreatedAt FROM SprayHeadInventory WHERE 1=0"
If Err.Number <> 0 Then Err.Clear : conn.Execute "ALTER TABLE SprayHeadInventory ADD CreatedAt DATETIME DEFAULT GETDATE()"
On Error GoTo 0
...
If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
    ...
    If action = "add" Then
        Dim pName, pCode, pType, pMaterial, pSpec, pUnit, pStock, pSafety, pPrice, pSupplier, pNotes
        pName = SafeSQL(Trim(Request.Form("item_name")))
        pCode = SafeSQL(Trim(Request.Form("item_code")))
        pType = SafeSQL(Trim(Request.Form("spray_type")))
        ...
        If pName <> "" Then
            conn.Execute "INSERT INTO SprayHeadInventory (ItemName, ItemCode, SprayType, Material, Specification, Unit, StockQty, SafetyStock, UnitPrice, WeightedUnitCost, SupplierID, Notes) VALUES ('" & _
                pName & "','" & pCode & "','" & pType & "','" & pMaterial & "','" & pSpec & "','" & pUnit & "'," & pStock & "," & pSafety & "," & pPrice & "," & pPrice & "," & IIf(pSupplier>0, pSupplier, "NULL") & ",'" & pNotes & "')"
            Response.Redirect "sprayhead_purchase.asp?msg=添加成功"
            Response.End
        End If
    ElseIf action = "edit" Then
        Dim eID : eID = SafeNum(Request.Form("sprayhead_id"))
        pName = SafeSQL(Trim(Request.Form("item_name")))
        pCode = SafeSQL(Trim(Request.Form("item_code")))
        pType = SafeSQL(Trim(Request.Form("spray_type")))
        pMaterial = SafeSQL(Trim(Request.Form("material")))
        ...
        If eID > 0 And pName <> "" Then
            conn.Execute "UPDATE SprayHeadInventory SET ItemName='" & pName & "', ItemCode='" & pCode & "', SprayType='" & pType & "', Material='" & pMaterial & "', Specification='" & pSpec & "', Unit='" & pUnit & "', SafetyStock=" & pSafety & ", UnitPrice=" & pPrice & ", SupplierID=" & IIf(pSupplier>0, pSupplier, "NULL") & ", Notes='" & pNotes & "', UpdatedAt=GETDATE() WHERE SprayHeadID=" & eID
            Response.Redirect "sprayhead_purchase.asp?msg=更新成功"
            Response.End
        End If
    ElseIf action = "restock" Then
        Dim rID : rID = SafeNum(Request.Form("sprayhead_id"))
        Dim rQty : rQty = SafeNum(Request.Form("restock_qty"))
        If rID > 0 And rQty > 0 Then
            conn.Execute "UPDATE SprayHeadInventory SET StockQty = StockQty + " & rQty & ", UpdatedAt=GETDATE() WHERE SprayHeadID=" & rID
            Response.Redirect "sprayhead_purchase.asp?msg=入库成功"
            Response.End
        End If
    End If
End If
...
totalItems = GetScalar("SELECT COUNT(*) FROM SprayHeadInventory WHERE IsActive=1")
totalStock = GetScalar("SELECT SUM(StockQty) FROM SprayHeadInventory WHERE IsActive=1")
lowStockCount = GetScalar("SELECT COUNT(*) FROM SprayHeadInventory WHERE IsActive=1 AND StockQty <= SafetyStock AND SafetyStock > 0")
totalValue = SafeNum(GetScalar("SELECT SUM(StockQty * UnitPrice) FROM SprayHeadInventory WHERE IsActive=1"))

' ========== 关联采购订单 ==========
Dim relatedPOCount, relatedPOAmount
relatedPOCount = GetScalar("SELECT COUNT(*) FROM PurchaseOrders WHERE OrderType='SprayHead' AND Status IN ('Ordered','PartialReceived','Received')")
relatedPOAmount = SafeNum(GetScalar("SELECT SUM(CAST(ISNULL(TotalAmount,0) AS FLOAT)) FROM PurchaseOrders WHERE OrderType='SprayHead' AND Month(OrderDate)=Month(GETDATE()) AND Year(OrderDate)=Year(GETDATE())"))

' ========== 获取喷头列表 ==========
Dim rsInv
Set rsInv = conn.Execute("SELECT p.*, s.SupplierName FROM SprayHeadInventory p LEFT JOIN Suppliers s ON p.SupplierID = s.SupplierID WHERE p.IsActive=1 ORDER BY p.ItemName")
%>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>喷头采购 - 采购管理中心</title>
...
<body data-theme="purchase-dark">
    ...
    <div class="main-content">
        <div class="page-header">
            <h2 class="page-title"><i class="fas fa-atom" style="color:#E91E63;"></i> 喷头采购管理</h2>
            <div class="breadcrumb">
                <a href="index.asp">采购中心</a> / <span>喷头采购</span>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-label"><i class="fas fa-atom" style="color:#E91E63;"></i> 喷头品类</div>
                <div class="stat-value"><%= totalItems %></div>
            </div>
            <div class="stat-card">
                <div class="stat-label"><i class="fas fa-layer-group" style="color:#E91E63;"></i> 总库存量</div>
            ...
            <div class="stat-card">
                <div class="stat-label"><i class="fas fa-yen-sign" style="color:#E91E63;"></i> 库存总值</div>
                <div class="stat-value">¥<%= FormatNumber(totalValue, 0) %></div>
            </div>
            <div class="stat-card">
                <div class="stat-label"><i class="fas fa-file-invoice" style="color:#E91E63;"></i> 进行中采购</div>
        ...
        <!-- 工具栏 -->
        <div class="toolbar">
            <button class="btn btn-primary" onclick="openAddModal()"><i class="fas fa-plus"></i> 新增喷头</button>
            <a href="purchase_orders.asp?order_type=SprayHead" class="btn btn-outline"><i class="fas fa-file-invoice"></i> 喷头采购订单</a>
            <a href="restock_order.asp?tab=SprayHead" class="btn btn-outline"><i class="fas fa-bolt"></i> 智能补货</a>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                %>
                <tr><td colspan="7" class="empty-row"><i class="fas fa-inbox"></i> 暂无喷头库存数据</td></tr>
                <%
                Else
                    Do While Not rsInv.EOF
                        Dim pID : pID = rsInv("SprayHeadID")
                ...
                <tr>
                    ...
                    <td><%= Server.HTMLEncode(rsInv("SprayType") & "") %></td>
                    ...
                    <td style="text-align:center;">
                        <button class="btn btn-outline btn--sm" onclick="openEditModal(<%= pID %>, '<%= Server.HTMLEncode(rsInv("ItemName") & "") %>', '<%= Server.HTMLEncode(rsInv("ItemCode") & "") %>', '<%= Server.HTMLEncode(rsInv("SprayType") & "") %>', '<%= Server.HTMLEncode(rsInv("Material") & "") %>', '<%= Server.HTMLEncode(rsInv("Specification") & "") %>', '<%= Server.HTMLEncode(rsInv("Unit") & "") %>', <%= pSafety %>, <%= FormatNumber(SafeNum(rsInv("UnitPrice")), 2) %>, '<%= Server.HTMLEncode(rsInv("SupplierID") & "") %>', '<%= Server.HTMLEncode(rsInv("Notes") & "") %>')"><i class="fas fa-edit"></i></button>
    ...
    <div class="modal" id="editModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 id="modalTitle"><i class="fas fa-plus-circle"></i> 新增喷头</h3>
                <button class="modal-close" onclick="closeModal('editModal')">&times;</button>
            </div>
            <form method="post">
                <input type="hidden" name="action" id="formAction" value="add">
                <input type="hidden" name="sprayhead_id" id="editSprayHeadID">
                <div class="form-row">
                    <div class="form-group">
                        <label>喷头名称 <span style="color:#F44336;">*</span></label>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>喷头类型</label>
                        <select name="spray_type" id="editType">
                            <option value="Mist">雾化喷头</option>
                            <option value="Pump">按压喷头</option>
                            <option value="PumpHead">泵头</option>
                            <option value="Trigger">扳机喷头</option>
                            <option value="Crimp">轧口喷头</option>
                            <option value="FineMist">细雾喷头</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="material" id="editMaterial">
                            ...
                            <option value="StainlessSteel">不锈钢</option>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>规格尺寸</label>
                        <input type="text" name="specification" id="editSpec" placeholder="如 18mm/400 或 20/410">
                    </div>
                    <div class="form-group">
                        <label>单位</label>
    ...
    <div class="modal" id="restockModal">
        <div class="modal-content">
            ...
            <form method="post">
                ...
                <input type="hidden" name="sprayhead_id" id="restockID">
                <div class="form-group">
                    <label>喷头名称</label>
    ...
    <script>
        function openAddModal() {
            ...
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle"></i> 新增喷头';
            document.getElementById('editSprayHeadID').value = '';
            document.getElementById('editName').value = '';
            document.getElementById('editCode').value = '';
            document.getElementById('editType').value = 'Mist';
        ...
        function openEditModal(id, name, code, type, material, spec, unit, safety, price, supplier, notes) {
            document.getElementById('formAction').value = 'edit';
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit"></i> 编辑喷头';
            document.getElementById('editSprayHeadID').value = id;
