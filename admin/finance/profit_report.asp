...
' 12.5 V21: 系统记账净利润——以 Orders.ProfitAmount 为权威口径（含 ExpenseAmount），去除两套口径歧义
Dim netProfitCanonical, netProfitRate
netProfitCanonical = GetScalar("SELECT CAST(ISNULL(SUM(ProfitAmount),0) AS FLOAT) FROM Orders WHERE OrderDate >= '" & safeStart & "' AND OrderDate <= '" & safeEnd & "' AND Status IN ('Paid','Processing','Shipped','Completed')")
If IsNull(netProfitCanonical) Or netProfitCanonical = "" Then netProfitCanonical = 0
If SafeNum(netSales) > 0 Then netProfitRate = (SafeNum(netProfitCanonical) / SafeNum(netSales)) * 100 Else netProfitRate = 0
Dim npClass, npSign, npLight, npStatus
If SafeNum(netProfitCanonical) >= 0 Then npClass = "positive" Else npClass = "negative"
If SafeNum(netProfitCanonical) >= 0 Then npSign = "" Else npSign = "-"
If SafeNum(netProfitCanonical) > 0 Then npLight = "green" Else npLight = "red"
If SafeNum(netProfitCanonical) > 0 Then npStatus = "盈利" Else npStatus = "亏损"
        ...
        <div class="report-section">
            ...
            <table class="profit-table">
                ...
                <tbody>
                    ...
                    <tr class="highlight-row">
                        <td><i class="fas fa-wallet" style="color: #4CAF50;"></i> 系统记账净利润 <span style="font-size:11px;color:#888;">(权威口径·含分摊费用)</span></td>
                        <td class="amount <%= npClass %>"><%= npSign %><%= FormatNumber(Abs(SafeNum(netProfitCanonical)), 2) %></td>
                        <td class="rate"><%= FormatNumber(SafeNum(netProfitRate), 1) %>%</td>
                        <td>
                            <div class="traffic-light">
                                <span class="light <%= npLight %>"></span>
                                <%= npStatus %>
                            </div>
                        </td>
                    </tr>
