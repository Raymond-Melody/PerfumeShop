...
If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
    ...
    If Not ValidateCSRFToken() Then
        Response.Write "<script>alert('" & T("user_login_csrf_fail", Empty) & "'); history.back();</script>"
        Response.End
    End If
    ...
    If action = "add" Or action = "edit" Then
        ...
        If action = "add" Then
            ...
            If newId > 0 Then
                Response.Write "<script>alert('" & T("user_address_save_success", Empty) & "'); location.href='addresses.asp';</script>"
            Else
                Response.Write "<script>alert('" & T("user_address_save_fail", Empty) & "'); location.href='addresses.asp';</script>"
            End If
        ElseIf action = "edit" Then
            Dim addressId : addressId = Request.Form("addressId")
            If Not IsNumeric(addressId) Then
                Response.Write "<script>alert('" & T("user_address_invalid_id", Empty) & "'); history.back();</script>"
                Response.End
            End If
    ...
    ElseIf action = "delete" Then
        ...
        If DAL_Users_DeleteAddress(CLng(deleteId), Session("UserID")) Then
            Response.Write "<script>alert('" & T("user_address_delete_success", Empty) & "'); location.href='addresses.asp';</script>"
        Else
            Response.Write "<script>alert('" & T("user_address_delete_fail", Empty) & "'); location.href='addresses.asp';</script>"
        End If
    ElseIf action = "setDefault" Then
        ...
        If DAL_Users_SetDefaultAddress(CLng(setAddressId), Session("UserID")) Then
            Response.Write "<script>alert('" & T("user_address_set_default_success", Empty) & "'); location.href='addresses.asp';</script>"
        Else
            Response.Write "<script>alert('" & T("user_address_set_default_fail", Empty) & "'); location.href='addresses.asp';</script>"
        End If
    End If
End If
