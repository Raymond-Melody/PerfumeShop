...
<div class="breadcrumb">
    <div class="container">
        ...
        <span><% If FEATURE_I18N Then %><%= T("user_nav_center", Empty) %><% Else %>个人中心<% End If %></span>
    </div>
</div>
    ...
    <div class="user-center">
        ...
        <div class="user-main">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-label"><% If FEATURE_I18N Then %><%= T("user_index_order_count", Empty) %><% Else %>订单数量<% End If %></span>
                    </div>
                </div>
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-label"><% If FEATURE_I18N Then %><%= T("user_index_total_spent", Empty) %><% Else %>累计消费<% End If %></span>
                    </div>
                </div>
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-label"><% If FEATURE_I18N Then %><%= T("user_index_points_balance", Empty) %><% Else %>积分余额<% End If %></span>
                    </div>
                </div>
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        ...
                        <span class="stat-label"><% If FEATURE_I18N Then %><%= T("user_index_coupons", Empty) %><% Else %>优惠券<% End If %></span>
                    </div>
                </div>
            </div>
            ...
            <div class="quick-actions">
                <h2><% If FEATURE_I18N Then %><%= T("user_index_quick_actions", Empty) %><% Else %>快捷操作<% End If %></h2>
                <div class="action-grid">
                    <a href="/products.asp" class="action-item">
                        <i class="fas fa-spray-can"></i>
                        <span><% If FEATURE_I18N Then %><%= T("user_index_browse", Empty) %><% Else %>选购香水<% End If %></span>
                    </a>
                    <a href="/customize.asp" class="action-item">
                        <i class="fas fa-magic"></i>
                        <span><% If FEATURE_I18N Then %><%= T("user_index_customize", Empty) %><% Else %>开始定制<% End If %></span>
                    </a>
                    <a href="/cart.asp" class="action-item">
                        <i class="fas fa-shopping-cart"></i>
                        <span><% If FEATURE_I18N Then %><%= T("user_index_my_cart", Empty) %><% Else %>我的购物车<% End If %></span>
                    </a>
                    <a href="/user/orders.asp" class="action-item">
                        <i class="fas fa-truck"></i>
                        <span><% If FEATURE_I18N Then %><%= T("user_index_track", Empty) %><% Else %>查看物流<% End If %></span>
                    </a>
                </div>
            </div>
            ...
            <div class="referral-section" id="referral">
                <div class="section-header">
                    <h2><i class="fas fa-user-friends"></i> <% If FEATURE_I18N Then %><%= T("user_nav_referral", Empty) %><% Else %>推荐好友<% End If %></h2>
                    <span style="font-size:13px;color:#999;"><% If FEATURE_I18N Then %><%= T("user_referral_invite_friends", Empty) %><% Else %>邀请好友加入，一起享受香氛定制<% End If %></span>
                </div>
                ...
                <div class="referral-stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:15px;margin-bottom:20px;">
                    <div class="stat-mini" style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;">
                        ...
                        <div style="font-size:12px;color:#888;"><% If FEATURE_I18N Then %><%= T("user_referral_today_generated", Empty) %><% Else %>今日生成<% End If %></div>
                    </div>
                    <div class="stat-mini" style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;">
                        <div style="font-size:24px;font-weight:bold;color:#333;"><%= referralStats("activeLinks") %></div>
                        <div style="font-size:12px;color:#888;"><% If FEATURE_I18N Then %><%= T("user_referral_active_links", Empty) %><% Else %>有效链接<% End If %></div>
                    </div>
                    <div class="stat-mini" style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;">
                        <div style="font-size:24px;font-weight:bold;color:#4CAF50;"><%= referralStats("totalInvitees") %></div>
                        <div style="font-size:12px;color:#888;"><% If FEATURE_I18N Then %><%= T("user_referral_invited", Empty) %><% Else %>已邀请<% End If %></div>
                    </div>
                    <div class="stat-mini" style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;">
                        <div style="font-size:24px;font-weight:bold;color:#999;"><%= 5 - CLng(referralStats("todayLinks")) %></div>
                        <div style="font-size:12px;color:#888;"><% If FEATURE_I18N Then %><%= T("user_referral_today_remaining", Empty) %><% Else %>今日剩余<% End If %></div>
                    </div>
                </div>
                ...
                <form method="post" style="margin-bottom:20px;" id="genLinkForm">
                    ...
                    <button type="submit" class="btn btn-primary" style="padding:12px 24px;">
                        <i class="fas fa-plus-circle"></i> <% If FEATURE_I18N Then %><%= T("user_referral_generate_btn", Empty) %><% Else %>生成推荐链接<% End If %>
                    </button>
                    <span style="font-size:12px;color:#999;margin-left:10px;"><% If FEATURE_I18N Then %><%= T("user_referral_valid_hint", Empty) %><% Else %>有效期30天 · 每日限5个<% End If %></span>
                </form>
                <% Else %>
                ...
                <div class="referral-links-list">
                    <h4 style="margin-bottom:12px;"><i class="fas fa-link" style="color:#4CAF50;"></i> <% If FEATURE_I18N Then %><%= T("user_referral_active_links_title", Empty) %><% Else %>有效推荐链接<% End If %></h4>
                    <%
                        ...
                        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                            <div style="flex:1;min-width:200px;">
                                <div style="font-size:11px;color:#999;margin-bottom:4px;">
                                    <% If FEATURE_I18N Then %><%= T("user_referral_generated_at", Empty) %><% Else %>生成<% End If %>: <%= SafeFormatDateTime(linkCreatedAt, 2) %> · <% If FEATURE_I18N Then %><%= T("user_referral_expires", Empty) %><% Else %>过期<% End If %>: <%= SafeFormatDateTime(linkExpiresAt, 2) %> · <% If FEATURE_I18N Then %><%= T("user_referral_used", Empty) %><% Else %>已用<% End If %> <%= linkUsed %>/<%= linkMax %>
                                </div>
                                <% If linkUrl <> "" Then %>
                                <div style="display:flex;align-items:center;gap:6px;">
                                    ...
                                    <button type="button" class="btn btn-sm" onclick="copyRefLink(<%= linkIndex %>)" style="background:#4CAF50;color:#fff;white-space:nowrap;padding:6px 12px;">
                                        <i class="fas fa-copy"></i> <% If FEATURE_I18N Then %><%= T("user_referral_copy", Empty) %><% Else %>复制<% End If %>
                                    </button>
                                    <button type="button" class="btn btn-sm" onclick="toggleShareMenu(<%= linkIndex %>)" style="background:#2196F3;color:#fff;white-space:nowrap;padding:6px 12px;" id="shareBtn<%= linkIndex %>">
                                        <i class="fas fa-share-alt"></i> <% If FEATURE_I18N Then %><%= T("user_referral_share", Empty) %><% Else %>分享<% End If %>
                                    </button>
                                </div>
                                <div id="shareMenu<%= linkIndex %>" style="display:none;margin-top:8px;padding:10px;background:#fff;border:1px solid #e0e0e0;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                                    <div style="font-size:12px;color:#888;margin-bottom:8px;"><% If FEATURE_I18N Then %><%= T("user_referral_share_to", Empty) %><% Else %>分享到社交平台：<% End If %></div>
                                ...
                                <% Else %>
                                <span style="color:#999;font-size:12px;"><% If FEATURE_I18N Then %><%= T("user_referral_link_expired", Empty) %><% Else %>链接已失效<% End If %></span>
                                <% End If %>
                            </div>
                        </div>
                    </div>
                ...
                function copyRefLink(idx) {
                    ...
                    if (navigator.clipboard && navigator.clipboard.writeText) {
                        navigator.clipboard.writeText(input.value).then(function() {
                            showToast('<% If FEATURE_I18N Then %><%= T("user_referral_copied", Empty) %><% Else %>推荐链接已复制到剪贴板<% End If %>');
                        });
                    } else {
                        document.execCommand('copy');
                        showToast('<% If FEATURE_I18N Then %><%= T("user_referral_copied", Empty) %><% Else %>推荐链接已复制<% End If %>');
                    }
                }
                ...
                function shareWeibo(idx) {
                    ...
                    var title = encodeURIComponent('<% If FEATURE_I18N Then %><%= T("user_referral_share_desc", Empty) %><% Else %>我在香氛定制平台发现了超棒的定制香水，快来加入我们吧！<% End If %>');
                ...
                function shareQQ(idx) {
                    var url = getLinkUrl(idx);
                    var title = encodeURIComponent('<% If FEATURE_I18N Then %><%= T("user_referral_share_qq_title", Empty) %><% Else %>香氛定制 - 会员推荐注册<% End If %>');
                    var desc = encodeURIComponent('<% If FEATURE_I18N Then %><%= T("user_referral_share_desc", Empty) %><% Else %>我在香氛定制平台发现了超棒的定制香水，快来加入我们吧！<% End If %>');
                ...
                function shareWeChat(idx) {
                    ...
                    if (navigator.clipboard && navigator.clipboard.writeText) {
                        navigator.clipboard.writeText(url).then(function() {
                            showToast('<% If FEATURE_I18N Then %><%= T("user_referral_wechat_copied", Empty) %><% Else %>链接已复制，请打开微信粘贴发送给好友<% End If %>');
                        });
                    } else {
                        var input = document.getElementById('refLink' + idx);
                        input.select();
                ...
                <div style="text-align:center;padding:30px;color:#999;background:#f8f9fa;border-radius:8px;">
                    ...
                    <p><% If FEATURE_I18N Then %><%= T("user_referral_no_links", Empty) %><% Else %>暂无有效推荐链接，点击上方按钮生成<% End If %></p>
                </div>
                <%
                End If
                Set rsLinks = Nothing
            ...
            <div class="recent-orders">
                <div class="section-header">
                    <h2><% If FEATURE_I18N Then %><%= T("user_index_recent_orders", Empty) %><% Else %>最近订单<% End If %></h2>
                    <a href="/user/orders.asp"><% If FEATURE_I18N Then %><%= T("user_index_view_all", Empty) %><% Else %>查看全部<% End If %> <i class="fas fa-arrow-right"></i></a>
                </div>
                ...
                <div class="orders-list">
                    ...
                    <div class="order-item">
                        <div class="order-info">
                            <span class="order-no"><% If FEATURE_I18N Then %><%= T("user_index_order_no", Empty) %><% Else %>订单号<% End If %>: <%= rsOrders("OrderNo") %></span>
                        ...
                        <div class="order-action">
                            <a href="/user/order_detail.asp?order_id=<%= rsOrders("OrderID") %>" class="btn btn-sm btn-outline"><% If FEATURE_I18N Then %><%= T("user_orders_view_detail", Empty) %><% Else %>查看详情<% End If %></a>
                        </div>
                    </div>
                    <%
                        rsOrders.MoveNext
                ...
                <div class="empty-orders">
                    ...
                    <p><% If FEATURE_I18N Then %><%= T("user_index_no_orders", Empty) %><% Else %>暂无订单记录<% End If %></p>
                    <a href="/products.asp" class="btn btn-primary"><% If FEATURE_I18N Then %><%= T("user_index_go_shop", Empty) %><% Else %>去选购<% End If %></a>
                </div>
                <%
                End If
                %>
