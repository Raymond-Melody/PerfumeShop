    ...
    <div class="auth-container">
        <div class="auth-card">
            ...
            <div class="auth-header">
                <h1><% If FEATURE_I18N Then %><%= T("user_reset_link_invalid", Empty) %><% Else %>链接无效<% End If %></h1>
                <p><%= HTMLEncode(errorMsg) %></p>
            </div>
            <div class="auth-footer">
                <p><a href="/user/forgot.asp" class="btn btn-primary"><% If FEATURE_I18N Then %><%= T("user_reset_reapply", Empty) %><% Else %>重新申请重置<% End If %></a></p>
                <p><a href="/user/login.asp"><% If FEATURE_I18N Then %><%= T("user_forgot_back_link", Empty) %><% Else %>返回登录<% End If %></a></p>
            </div>
            
            <% ElseIf step = "done" Then %>
            <div class="auth-header">
                <h1><% If FEATURE_I18N Then %><%= T("user_reset_success_title", Empty) %><% Else %>重置成功<% End If %></h1>
                <p><%= HTMLEncode(successMsg) %></p>
            </div>
            <div class="auth-footer">
                <p><a href="/user/login.asp" class="btn btn-primary"><% If FEATURE_I18N Then %><%= T("user_reset_goto_login", Empty) %><% Else %>前往登录<% End If %></a></p>
            </div>
            
            <% Else %>
            <div class="auth-header">
                <h1><% If FEATURE_I18N Then %><%= T("user_reset_set_new", Empty) %><% Else %>设置新密码<% End If %></h1>
                <p><% If FEATURE_I18N Then %><%= T("user_reset_set_for_account", Array(rsToken("Username"))) %><% Else %>为账户 <strong><%= HTMLEncode(rsToken("Username")) %></strong> 设置新密码<% End If %></p>
            </div>
            ...
            <form method="post" class="auth-form">
                <div class="form-group">
                    <label for="new_password"><i class="fas fa-lock"></i> <% If FEATURE_I18N Then %><%= T("user_reset_new", Empty) %><% Else %>新密码<% End If %></label>
                    <input type="password" id="new_password" name="new_password" placeholder="<% If FEATURE_I18N Then %><%= T("user_reset_new_placeholder", Empty) %><% Else %>至少6个字符<% End If %>" required minlength="6">
                </div>
                
                <div class="form-group">
                    <label for="confirm_password"><i class="fas fa-lock"></i> <% If FEATURE_I18N Then %><%= T("user_reset_confirm", Empty) %><% Else %>确认新密码<% End If %></label>
                    <input type="password" id="confirm_password" name="confirm_password" placeholder="<% If FEATURE_I18N Then %><%= T("user_reset_confirm_placeholder", Empty) %><% Else %>再次输入新密码<% End If %>" required minlength="6">
                </div>
                
                <button type="submit" class="btn btn-primary btn-lg btn-block">
                    <i class="fas fa-key"></i> <% If FEATURE_I18N Then %><%= T("user_reset_btn", Empty) %><% Else %>重置密码<% End If %>
                </button>
            </form>
